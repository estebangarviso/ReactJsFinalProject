/**
 * @author nowaalex
 * @link https://github.com/nowaalex/use-axios-progress-bar
 */
...
const useAxiosProgressBar = ({ axiosInstance = axios, delay = 200 } = {}) => {
  ...
  useEffect(() => {
    ...
    const requestInterceptor = request.use((config) => {
      ...
      const getProgressUpdateCallback =
        (prefix) =>
        ({ total, loaded }) => {
      ...
      if (
        ...
        config.transformResponse instanceof Array
      ) {
      ...
      config.onUploadProgress = wrapEvent(
        config.onUploadProgress,
        getProgressUpdateCallback('u')
      );
      config.onDownloadProgress = wrapEvent(
        config.onDownloadProgress,
        getProgressUpdateCallback('d')
      );
